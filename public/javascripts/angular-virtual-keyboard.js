    ...
    if (self.VKI_imageURI) {
      var keybut = document.createElement('img');
          ...
          keybut.onclick = function(e) {
            ...
            console.info('>>> VKI_show:', this.elem);
            self.VKI_show(this.elem);
          };
      elem.parentNode.insertBefore(keybut, (elem.dir == "rtl") ? elem : elem.nextSibling);
    } else {
      elem.onfocus = function() {
        console.info('>>> onfocus:', elem);
        if (self.VKI_target != this) {
          if (self.VKI_target) self.VKI_close(false);
          console.info('>>> show:', elem);
          self.VKI_show(this);
        }
      };
      elem.onclick = function() {
        console.info('>>> onclick:', elem);
        if (!self.VKI_target) {
            console.info('>>> show:', elem);
            self.VKI_show(this);
        }
...
console.info('>>>>> show:', elem);
    if (!this.VKI_target) {
console.info('>>>>> elem:', elem, 'VKI_target:', this.VKI_target);
...
console.info('>>>>> appendChild:', this.VKI_keyboard);
      document.body.appendChild(this.VKI_keyboard);
      this.VKI_keyboard.style.position = this.VKI_target.keyboardPosition;
      if (this.VKI_isOpera) this.VKI_keyboard.reflow();

      console.info('>>>>> VKI_position:', elem);
      this.VKI_position(true);
      if (self.VKI_isMoz || self.VKI_isWebKit) this.VKI_position(true);
      console.info('>>>>> blur:', elem);
      this.VKI_target.blur();
      console.info('>>>>> focus:', elem);
      this.VKI_target.focus();
...
console.info('>>>>> VKI_position:', top, left, bottom, right);
            self.VKI_keyboard.style.display = (top < 0 || left < 0 || bottom < 0 || right < 0) ? "none" : "";
console.info('>>>>> VKI_position style.display:', self.VKI_keyboard.style.display);
